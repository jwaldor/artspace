...
function Post({ post }: { post: PostType }) {
    ...
    const isLiked = useCallback(() => {
        return post.likes.find((like) => like.id === user?.user?.id);
            ...
            <div className="flex justify-end items-center mt-2">
                <div>
                    <button onClick={async () => {
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 inline-block" viewBox="0 0 20 20" fill={isLiked() ? "red" : "none"} stroke="black" style={{ transform: 'translateY(-2px) translateX(1px)' }}>
